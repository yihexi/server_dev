...
这中方案没有什么知识点，客户端所有的跨域访问需求都让自己的服务器转发，取消跨域访问。这种方案适用于业务请求转发，在没有流量压力的时候挺好，尤其是使用NodeJs做代理后端对于前端更容易上手，有一个薄的后端逻辑更加灵活可控，和业务后端解耦更彻底。感觉对于资源的访问，尤其是有CDN什么的，使用这种方案就失去优势了，大型网站应该不会这么干。
...
WebSocket是通过HTTP协议发起连接的，在发起连接时，有一个HTTP头Origin，表明了该请求的请求源：
...
服务端接收到这个HTTP请求时，如果Origin的域名允许连接的，就是转换HTTP协议到WebSocket。

> //TODO:如何防止Origin伪造，在WebSocket文章中介绍。
...
前边的3种解决方案都有点绕过跨域的意思，而CORS才是直接面对跨域问题的解决方案，因为CORS叫做跨源资源分享（Cross-Origin Resource Sharing）。它是W3C标准，是跨源AJAX请求的根本解决方法。相比JSONP只能发`GET`请求，CORS允许任何类型的请求。
